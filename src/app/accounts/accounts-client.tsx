...
import { Landmark, PlusCircle, Trash, Wallet, Coins, CreditCard } from "lucide-react";
...
const accountIcons = {
    ...
    "Credit Card": <CreditCard className="h-8 w-8 text-muted-foreground" />
}

const AccountCard = ({ account, onDelete }: { account: Account, onDelete: (id: string) => void }) => {
    const isCreditCard = account.type === 'Credit Card';
    const availableCredit = isCreditCard && account.creditLimit ? account.creditLimit - account.balance : 0;
    ...
    <Card className="card-glassmorphic flex flex-col justify-between min-h-[150px] rounded-xl">
        ...
        <div className="p-2 flex justify-end items-center text-muted-foreground">
             {isCreditCard ? (
                <span className="text-xs mr-auto ml-2">Disponible: {formatCurrency(availableCredit)}</span>
             ) : (
                <span className="w-2 h-2 rounded-full bg-green-500 mr-auto ml-2"></span>
             )}
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); onDelete(account.id);}}>
...
export const AccountsClient: React.FC<AccountsClientProps> = ({ data, onAccountChange }) => {
    ...
    const totalPatrimony = data.filter(acc => acc.type !== 'Credit Card').reduce((sum, account) => sum + account.balance, 0);
    ...
    const creditCardAccounts = data.filter(a => a.type === 'Credit Card');
        ...
        <div className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <Card className="card-glassmorphic rounded-xl">
                    <CardHeader>
                        <CardTitle className="font-headline text-lg">Patrimonio Neto</CardTitle>
                        <CardDescription>Suma de cuentas (sin incluir deudas de tarjetas)</CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <div className="space-y-6">
                ...
                {creditCardAccounts.length > 0 && (
                    <div className="space-y-4">
                        <h2 className="text-xl font-bold font-headline text-muted-foreground">Tarjetas de Crédito</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {creditCardAccounts.map(account => <AccountCard key={account.id} account={account} onDelete={handleDeleteRequest} />)}
                        </div>
                    </div>
                )}
                {/* Bank Accounts */}
